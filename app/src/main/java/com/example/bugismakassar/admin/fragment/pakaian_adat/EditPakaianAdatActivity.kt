...
class EditPakaianAdatActivity : AppCompatActivity() {
    private lateinit var binding: ActivityEditPakaianAdatBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEditPakaianAdatBinding.inflate(layoutInflater)
        setContentView(binding.root)

        database = FirebaseDatabase.getInstance().reference.child("Pakaian Adat")
        ...
        binding.btnUpdate.setOnClickListener {
            ...
            mediaData?.let { it1 ->
                storage.putFile(it1).addOnSuccessListener(OnSuccessListener { taskSnapshot ->
                    ...
                    Toast.makeText(this@EditPakaianAdatActivity,"Update Berhasil", Toast.LENGTH_SHORT).show()
                })
            }
        }
    }
    ...
    private fun updateDataToFirebaseDatabase(profileImageUrl: String, article: Article) {
        ...
        article.id?.let {
            database.child(it).setValue(articleData).addOnSuccessListener {
                ...
                Toast.makeText(this@EditPakaianAdatActivity, "Update Berhasil", Toast.LENGTH_SHORT).show()
                val intent = Intent(this@EditPakaianAdatActivity, AdminActivity::class.java)
                startActivity(intent)
            }
                .addOnFailureListener {
                    Toast.makeText(this@EditPakaianAdatActivity,"Update Gagal", Toast.LENGTH_SHORT).show()
                }
        }
    }
