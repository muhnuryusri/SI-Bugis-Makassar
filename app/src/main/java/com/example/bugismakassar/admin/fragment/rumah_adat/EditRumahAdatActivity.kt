...
class EditRumahAdatActivity : AppCompatActivity() {
    private lateinit var binding: ActivityEditRumahAdatBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEditRumahAdatBinding.inflate(layoutInflater)
        ...
        binding.btnUpdate.setOnClickListener {
            ...
            mediaData?.let { it1 ->
                storage.putFile(it1).addOnSuccessListener(OnSuccessListener { taskSnapshot ->
                    ...
                    Toast.makeText(this@EditRumahAdatActivity,"Update Berhasil", Toast.LENGTH_SHORT).show()
                })
            }
        }
    }
    ...
    private fun updateDataToFirebaseDatabase(profileImageUrl: String, article: Article) {
        ...
        article.id?.let {
            database.child(it).setValue(articleData).addOnSuccessListener {
                ...
                Toast.makeText(this@EditRumahAdatActivity, "Update Berhasil", Toast.LENGTH_SHORT).show()
                val intent = Intent(this@EditRumahAdatActivity, AdminActivity::class.java)
                startActivity(intent)
            }
                .addOnFailureListener {
                    Toast.makeText(this@EditRumahAdatActivity,"Update Gagal", Toast.LENGTH_SHORT).show()
                }
        }
    }
