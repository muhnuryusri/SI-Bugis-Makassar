...
import com.example.bugismakassar.admin.fragment.rumah_adat.EditRumahAdatActivity
...
class EditSejarahBugisAdapter (val context: Context?, private val listArticle: ArrayList<Article>) : RecyclerView.Adapter<RecyclerView.ViewHolder>()  {
    ...
    private inner class ImageViewHolder(private val binding: ListItemArticleEditBinding): RecyclerView.ViewHolder(binding.root) {
        ...
        init {
            binding.editArticle.setOnClickListener {
                val intent = Intent(context, EditSejarahBugisActivity::class.java)
                intent.putExtra(EditSejarahBugisActivity.EXTRA_ARTICLE, listArticle.get(position))
